...
- **Change-evidence calibration**: Completed the safety-gated λ sequence. λ=0.7 failed the IBL gate, λ=0.85 was dominated by λ=0.8, and λ=0.9 is the leading experimental combined profile. With `uncertain_retry` still enabled, λ=0.9 full control reaches PRL block-learning lift `+0.469` and optimal choice `0.706`; IBL full-control retry gap is `0.115` versus the historical flag-off `0.165`. The feature remains default off.
