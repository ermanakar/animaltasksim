...
- Current PRL result: `exploration_only` shows a reproducible hidden-contingency block-learning phenotype. A 10-condition, 50-run PRL scale sweep found that every full-control variant still suppresses most of that benefit, so scalar tuning alone is insufficient.
- Roadmap: use `scripts/prl_arbitration_diagnostic.py` to inspect reversal-window controller decomposition, test state-dependent arbitration without weakening the validated IBL retry claim, then add **Delayed Match-to-Sample** adaptive rollout only after its memory-specific fingerprint is defined. Keep all diagnostics compatible with the frozen shared pipeline.
